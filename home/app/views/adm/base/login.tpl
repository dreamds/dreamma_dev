<div id="login-wrapper">
    <div id="login-content">
        <div id="login-top" class="pngfix">
            <img class="pngfix" src="<?php echo HOME_DOMAIN ?>public/images/admin/login_title.png" alt="<?php echo ADM_TITLE; ?>" />
        </div>
        <form onsubmit="return false;">
            ...
            <p class="captcha">
                ...
                <a href="javascript:void(0)"><img style="vertical-align:middle;" src="<?php echo HOME_DOMAIN ?>adm_auth/captcha/?useage=backend&<?php echo microtime(true) ?>" title="点击换一张验证码"/></a>                
            </p>
...
$(document).ready(function(){
    ...
    $('.captcha img').click(function () {
        $(this).attr('src', '<?php echo HOME_DOMAIN ?>adm_auth/captcha/?useage=backend&' + Math.random());
        return false;
    }); 
})
...
function do_submit() {
    ...
    $.post('<?php echo HOME_DOMAIN; ?>adm_auth/login', $('form').serialize(), function (response) {
...
function get_sms_captcha()
{
    ...
    $.post('<?php echo HOME_DOMAIN; ?>adm_auth/get_sms_captcha', 'phone_number=' + phone_number, function (response) {
        if (true === response.success) {
...
function do_submit_sms() {
    ...
    $.post('<?php echo HOME_DOMAIN; ?>adm_auth/sms_login', $('form').serialize(), function (response) {
        if (true === response.success) {
            $.BKD.redirect(response.data);
        } else {
            if (response.data) {
